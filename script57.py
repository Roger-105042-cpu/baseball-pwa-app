...
# ------------------------------------------------------------------------------
st.sidebar.markdown("---")
with st.sidebar.expander("📖 拍攝與仰角量測指南"):
    st.markdown("""
### 🎥 方法 A：影像分析軟體法（最推薦、最準確）
* **下載工具**：使用免費且開源的運動分析軟體，例如 Kinovea 官方網站（電腦版）或手機 App（如 OnForm、Coach's Eye 概念工具）。
* **設定比例尺**：在畫面中放一個已知長度的物體（如 1 公尺的球棒）作為軟體的校準基準。
* **量測角度**：
  1. 找出「擊中球」的停頓影格。
  2. 往後播放 1-2 個影格，找出球飛出的點。
  3. 使用軟體內建的 **「角度標記工具」(Angle Tool)**。
  4. 一條線貼緊地面（水平線），另一條線連向球飛出的軌跡，軟體便會直接顯示仰角。

---

### 📐 方法 B：手動坐標計算法（數學原理）
如果您想手動計算，請截取球剛離棒後 1-2 格的畫面，並利用畫面中的像素或實際距離進行計算：
1. **標記兩點**：紀錄擊球點的坐標 $(X_1, Y_1)$，以及下一格球所在位置的坐標 $(X_2, Y_2)$。
2. **算出變化量**：
   * 水平移動距離 $\Delta X = X_2 - X_1$
   * 垂直上升高度 $\Delta Y = Y_2 - Y_1$
3. **帶入公式**：
   $$\text{擊球仰角} = \tan^{-1}\left(\frac{\Delta Y}{\Delta X}\right)$$
   *(例如：球離棒後，水平前進了 2 公尺，高度上升了 0.7 公尺，透過公式計算得出仰角約為 19.3 度。)*

---

### 💡 提高精準度的小技巧
* **背景參照物**：在打擊網或牆面上貼上簡單的水平與垂直膠帶作為參照線，量測時更容易抓準水平面。
* **避免入鏡透視誤差**：攝影機不要離打者太近。距離拉遠（如 5 公尺以上）並使用鏡頭光學變焦放大，可以有效減少畫面邊緣的「魚眼」或透視變形。
""")
